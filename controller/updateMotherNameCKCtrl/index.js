...
const controller = {
  updateMotherNameCK(request, response) {
    try {
      const { useridCK, upd_mother_name } = request.body;

      pool.db_MMFPROD.query(
        `select count(*) ada_ibu 
        ...
        (error, results) => {
          ...
          if (results.rows[0].ada_ibu != 0) {
            pool.db_MMFPROD.query(
              `update applicant_family_tbl set name =$2
              where  gender ='1' and relationship_type ='2' and applicant_id = $1`,
              [useridCK, upd_mother_name],
              (error, results) => {
                if (error) throw error;
                ...
                if (results.rowCount != 0) {
                  pool.db_MMFPROD.query(
                    ...
                    where  gender ='1' and relationship_type ='2' 
