...
const controller = {
  news(request, response) {
    ...
    try {
      pool.db_HCM.query(
        'SELECT berita_id, kategori_berita, tgl_input, ket_header, deskripsi, tgl_event_dr, tgl_event_sd, lokasi, images, deskripsi_pdf FROM trx_berita WHERE url_webview = $1',
        [query],
        (_, results) => {
