  ...
  async getHistManage(request, response) {
    try {
      ...
      const query = `with x as (
        ...
        aa.employee_id as nokar_pengaju 
        ...
        case  when current_date-a.approved_date=0 then 'Hari ini'
            ...
            when current_date-a.approved_date=7 then '7 Hari yang lalu'
            when current_date-a.approved_date>7 then to_char(a.approved_date,'DD Mon YYYY') 
        end Durasi_Waktu ,a.approved_date status_date, aa.golid, e.wage_name  as tipe_cuti, 'Izin' as tipe_filter,	               	
        aa.employee_id||'/'||to_char(a.approved_date,'YYYYMMDD')||'/'||trim(to_char(aa.sequence_no,'9999999999999999')) as 	nobukti, 
        aa.employee_id as nokar_pengaju
        from approval_structure_tbl a
        ...
        aa.request_no as nobukti,    aa.employee_id as nokar_pengaju
        from approval_structure_tbl a
        ...
        to_char(a.cor_absence_id,'9999999999999999') as no_urut, initcap(a.display_name) nama,
        case  when current_date-a.approval_date=0 then 'Hari ini'
            when current_date-a.approval_date=1 then 'Kemarin'
            when current_date-a.approval_date=2 then '2 Hari yang lalu'
            when current_date-a.approval_date=3 then '3 Hari yang lalu'
            when current_date-a.approval_date=4 then '4 Hari yang lalu'
            when current_date-a.approval_date=5 then '5 Hari yang lalu'
            when current_date-a.approval_date=6 then '6 Hari yang lalu'
            when current_date-a.approval_date=7 then '7 Hari yang lalu'
            when current_date-a.approval_date>7 then to_char(a.approval_date,'DD Mon YYYY') 
        end Durasi_Waktu ,a.approval_date as  status_date, a.cor_absence_id, 'Perbaikan Absen'  as tipe_cuti, 'Perbaikan Absen' as tipe_filter,	               	
        a.employee_id||'/'||to_char(a.request_date,'YYYYMMDD')||'/'||trim(to_char(a.cor_absence_id,'9999999999999999')) as nobukti, 
        a.employee_id as nokar_pengaju
        from correction_absence_hcm_h a
        where a.state_approval <>'Submitted'
        and a.approved_by =$1  and a.approval_date between (current_date -interval '1 days' * $2) and now()  
        )
        select * from x
