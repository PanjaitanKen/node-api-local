...
var controller = {
  getLongitude_Branch: function(request, response) {
    try {
      ...
      pool.db_MMFPROD.query("select work_location from emp_work_schedule_tbl a left join emp_work_location_tbl b on a.employee_id =b.employee_id where clocking_all ='Y' and current_date between a.valid_from and a.valid_to and a.employee_id= $1 ", [employee_id], (error, results) => {
        if (error) {
            throw error
        }
