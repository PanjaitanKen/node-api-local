...
const getBirthDatebyId = (request, response) => {
  ...
  pool.db.query(
    "SELECT to_char(birth_date,'MM-DD-YYYY') as birth_Date FROM person_tbl WHERE person_id = $1",
    [id],
    (error, results) => {
      if (error) {
        throw error;
...
const getLongitude_Branch = (request, response) => {
  pool.db.query(
    'select location_name, latitude,longitude from mark_location_tbl order by location_no asc',
    (error, results) => {
      if (error) {
        throw error;
      }
